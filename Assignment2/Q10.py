""" COSC121 Assignment 2, Question 10
    Author: J Cross
    23/05/2024
    """
...
def analytics_bargraph(total_seconds, sorted_tasks):
    """An attempt to split the analytics function into two - seems to work so far!
       This side prints the bar graph.
       """
    print("\nTotal Time (%)")
    max_percentage = max((task_info["time"].total_seconds() / total_seconds) * 100 for
                         task_id, task_info in sorted_tasks) if total_seconds > 0 else 0
    max_percentage = round(max_percentage)
    max_y_axis = min(int(max_percentage) + 5, 100)
    for i in range(max_y_axis, 0, -1):
        print(f"{i:>3} ", end=' ')
        for task_id, task_info in sorted_tasks:
            task_seconds = task_info["time"].total_seconds()
            percentage = round(((task_seconds / total_seconds) * 100 if total_seconds > 0 else 0))
            if percentage >= i:
                print('*********', end='')
            else:
                print('         ', end='')
