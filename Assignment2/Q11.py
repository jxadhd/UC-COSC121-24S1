...
def analytics_bargraph(total_seconds, sorted_tasks):
    """An attempt to split the analytics function into two - seems to work so far!
       Doing this refactors the analytics function to be short enough to keep
       Coderunner/Pylint happy :)) Prints a bargraph when called - now using
       matplotlib!"""
    # Extract task IDs and calculate percentages
    task_ids = [task_id for task_id, task_info in sorted_tasks]
    percentages = [round((task_info['time'].total_seconds() / total_seconds) * 100) for task_id,
                   task_info in sorted_tasks]
    plt.bar(task_ids, percentages)

    plt.xticks(np.arange(len(task_ids)), [f'{task_id}' for task_id in task_ids])
    plt.xlabel('Tasks')
